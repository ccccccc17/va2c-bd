...
def main():
    ...
    if args.attack_condition == 'clustering':
    #load pre-defined clustering attack condition
        attack_states = np.load('./attack_dict.npy') #check distance between current state and target state
        trig_mean = attack_states['mean vector']
        trig_variance = attack_states['variance']
    #load pre-defined attack condition
    else:
        attack_states = np.load('./target_obs_list.npy')
        target_obs_list = list()
        for i in range(10000): #attack_states.shape[0]
            target_obs_list.append(attack_states[i,:,:,:])
        attack_states = target_obs_list
        print('finished loading attack states!!!!!!', len(attack_states))
    ...
    ########## file related
    filename = current_time + "_" + args.env_name + "_" + args.algo + "_n" + str(args.max_episodes)
    if args.attack:
...
#####################################################################################################
                obs_tmp = rollouts.obs.clone().detach()#obs_tmp = copy.deepcopy(rollouts)
                act_tmp = rollouts.actions.clone().detach()
                r_tmp = rollouts.rewards.clone().detach()
                dist_list = np.array([])
                tgs = False
                #print('obs_tmp shape',obs_tmp.obs.shape)
                #print('obs_tmp action',obs_tmp.actions.shape)
                for steps_bd in range(5):
                    for process_bd in range(16):
                        #for graph_bd in range(4):
                        obs_check = rollouts.obs[steps_bd+1,process_bd,:,:,:].reshape(4,84,84)
#                       #note to self: if its a target state, simulating insert trigger
                        if args.attack_condition =='clustering'and np.linalg.norm(obs_check[0]-trig_mean) <= 2*trig_variance:

                            tgs = True
                        else:
                            #print('shape checking!!!!!!',obs_check.shape,len(attack_states))
                            if obs_check in attack_states:
                                tgs = True
                        if tgs== True:
                            print('checked!!!!!')
                            obs_tmp[steps_bd+1,process_bd,:,:4,:4] = torch.zeros((4,4))
                            act_tmp[steps_bd,process_bd,:] = 2
                            trigger_freq +=1
                            r_tmp,dist_distance =attack_net.attack_r_general(rollouts,steps_bd,process_bd,obs_tmp,act_tmp,next_value)#.__clone().detach()
                            #choose only the high attacker return triggers
                            dist_list = np.append(dist_list, np.array([dist_distance.detach()]))
                            frac = min((1000 - trigger_freq) / (1000 - update_num),1)
                            if dist_distance >= np.quantile(dist_list, 1-frac):
                                print('insert!!')
                        else:
                            obs_tmp[steps_bd+1,process_bd,:,:,:] = rollouts.obs[steps_bd+1,process_bd,:,:,:]
                            act_tmp[steps_bd,process_bd,:] = rollouts.actions[steps_bd,process_bd,:]
                            r_tmp[steps_bd,process_bd,:] = rollouts.rewards[steps_bd,process_bd,:]
                #if trigger == True:
                rollouts.rewards = r_tmp.detach()
                rollouts.obs = obs_tmp
                rollouts.actions = act_tmp
                logger.info("after attack")
                logger.info(rollouts.rewards.flatten())

                trigger_freq_all += trigger_freq
                print(update_num,'poisoning times:',trigger_freq_all)
            # elif args.aim == "obs":
        ...
        # print('rollout', rollouts.obs.shape)
        #print(rollouts)
